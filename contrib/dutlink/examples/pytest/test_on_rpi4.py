...
class TestResource(JumpstarterTest):
    ...
    def test_power_off_camera(self, client):
        client.dutlink.power.off()
        client.camera.write_snapshot("camera_off.jpeg")

    def test_power_on_camera(self, client):
        client.dutlink.power.on()
        time.sleep(1)
        client.camera.write_snapshot("camera_on.jpeg")
        client.dutlink.power.off()

    def test_power_on_hdmi(self, client):
        # check all the image snapshots through the rpi4 boot process
        client.dutlink.power.on()
        time.sleep(1)
        client.video.assert_snapshot("test_booting_empty_ok.jpeg")
        time.sleep(6)
        client.video.assert_snapshot("test_booting_rainbow_ok.jpeg")
        time.sleep(4)
        client.video.assert_snapshot("test_booting_raspberries_ok.jpeg")
        client.dutlink.power.off()

    def test_login_console_hdmi(self, client, shell):
        client.video.assert_snapshot("test_booted_ok.jpeg")
